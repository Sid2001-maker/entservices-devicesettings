"""
/**
 * @file TCID18_PortEnumerationConsistency.py
 * @brief L3 AVInput scenario testcase.
 *
 * @testcase TCID18_PortEnumerationConsistency
...
def run_test():
    ...
    if count is None:
        log_error("TCID18_PortEnumerationConsistency Failed ❌ (numberOfInputs invalid)")
        return False
    if count <= 0:
        log_error("TCID18_PortEnumerationConsistency Failed ❌ (device reports 0 HDMI input ports)")
        return False
    ...
    if devices is None:
        log_error("TCID18_PortEnumerationConsistency Failed ❌ (device list invalid)")
        return False

    log_info("Step 3: cross-validating topology")
    if len(devices) != count:
        log_error(
            f"TCID18_PortEnumerationConsistency Failed ❌ "
    ...
    if ids != list(range(count)):
        log_error(f"TCID18_PortEnumerationConsistency Failed ❌ (ids not contiguous 0..n-1: {ids})")
        return False
    ...
    for dev in devices:
        ...
        if not isinstance(dev.get("connected"), bool):
            log_error("TCID18_PortEnumerationConsistency Failed ❌ (connected not boolean)")
            return False

    log_success(f"✅ Topology consistent across both APIs: ids {ids}")

    elapsed_time = time.perf_counter() - start_time
    msg = "TCID18_PortEnumerationConsistency Passed ✅"
