"""
/**
 * @file TCID23_MultiPortIndependence.py
 * @brief L3 AVInput scenario testcase.
 *
 * @testcase TCID23_MultiPortIndependence
...
def run_test():
    ...
    if not count or count < 2:
        log_error(
            f"TCID23_MultiPortIndependence Failed ❌ "
    ...
    try:
        ...
        if not _provision(PORT_A, AVInputApis.EDID_VERSION_20, True):
            log_error("TCID23_MultiPortIndependence Failed ❌ (port A provisioning rejected)")
            return False

        log_info(f"Step 2: provision port {PORT_B} -> HDMI1.4 + ALLM false")
        if not _provision(PORT_B, AVInputApis.EDID_VERSION_14, False):
            log_error("TCID23_MultiPortIndependence Failed ❌ (port B provisioning rejected)")
            return False
        ...
        if ver_a != AVInputApis.EDID_VERSION_20:
            log_error(f"TCID23_MultiPortIndependence Failed ❌ (port A version leaked: {ver_a})")
            return False
        if ver_b != AVInputApis.EDID_VERSION_14:
            log_error(f"TCID23_MultiPortIndependence Failed ❌ (port B version leaked: {ver_b})")
            return False
        if allm_a is not True:
            log_error(f"TCID23_MultiPortIndependence Failed ❌ (port A ALLM leaked: {allm_a})")
            return False
        if allm_b is not False:
            log_error(f"TCID23_MultiPortIndependence Failed ❌ (port B ALLM leaked: {allm_b})")
            return False
    ...
    msg = "TCID23_MultiPortIndependence Passed ✅"
