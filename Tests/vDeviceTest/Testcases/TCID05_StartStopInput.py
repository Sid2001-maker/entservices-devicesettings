"""
/**
 * @file TCID05_StartStopInput.py
 * @brief L3 AVInput functional testcase.
 *
 * @testcase TCID05_StartStopInput
...
def run_test():
    ...
    if not is_ok(reg):
        log_error("TCID05_StartStopInput Failed ❌ (failed to register onInputStatusChanged)")
        return False
    ...
    try:
        ...
        if not is_ok(start_resp):
            log_error("TCID05_StartStopInput Failed ❌ (startInput did not succeed)")
            return False
        ...
        if not is_ok(stop_resp):
            log_error("TCID05_StartStopInput Failed ❌ (stopInput did not succeed)")
            return False
        ...
        if not is_ok(stop_again):
            log_error("TCID05_StartStopInput Failed ❌ (second stopInput not handled)")
            return False
    finally:
        send_curl_command(AVInputApis.stop_input(AVInputApis.TYPE_HDMI))

    elapsed_time = time.perf_counter() - start_time
    msg = "TCID05_StartStopInput Passed ✅"
