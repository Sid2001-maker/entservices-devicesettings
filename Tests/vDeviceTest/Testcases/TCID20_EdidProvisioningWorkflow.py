"""
/**
 * @file TCID20_EdidProvisioningWorkflow.py
 * @brief L3 AVInput scenario testcase.
 *
 * @testcase TCID20_EdidProvisioningWorkflow
...
def run_test():
    ...
    if not original_edid:
        log_error("TCID20_EdidProvisioningWorkflow Failed ❌ (readEDID returned no EDID)")
        return False

    decoded_len = _decoded_len(original_edid)
    if decoded_len is None:
        log_error("TCID20_EdidProvisioningWorkflow Failed ❌ (EDID is not valid base64)")
        return False
    if decoded_len == 0 or decoded_len % 128 != 0:
        log_error(
            f"TCID20_EdidProvisioningWorkflow Failed ❌ "
    ...
    try:
        ...
        if not result_success(set_resp):
            log_error("TCID20_EdidProvisioningWorkflow Failed ❌ (setEdidVersion rejected)")
            return False
        ...
        if parse_edid_version(ver_resp) != AVInputApis.EDID_VERSION_20:
            log_error("TCID20_EdidProvisioningWorkflow Failed ❌ (EDID version did not persist)")
            return False
        ...
        if not result_success(write_resp):
            log_error("TCID20_EdidProvisioningWorkflow Failed ❌ (writeEDID rejected)")
            return False
        ...
        if not final_edid or _decoded_len(final_edid) is None:
            log_error("TCID20_EdidProvisioningWorkflow Failed ❌ (post-write EDID unreadable)")
            return False
    ...
    msg = "TCID20_EdidProvisioningWorkflow Passed ✅"
