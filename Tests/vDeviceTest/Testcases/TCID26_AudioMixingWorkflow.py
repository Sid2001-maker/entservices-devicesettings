"""
/**
 * @file TCID26_AudioMixingWorkflow.py
 * @brief L3 AVInput scenario testcase.
 *
 * @testcase TCID26_AudioMixingWorkflow
...
def run_test():
    ...
    try:
        ...
        if not result_success(mixed_resp):
            log_error("TCID26_AudioMixingWorkflow Failed ❌ (mixed-audio start rejected)")
            return False
        ...
        for primary, source in MIXER_SWEEP:
            ...
            if not result_success(resp):
                log_error(
                    f"TCID26_AudioMixingWorkflow Failed ❌ "
        ...
        if not result_success(stop_resp):
            log_error("TCID26_AudioMixingWorkflow Failed ❌ (stopInput rejected)")
            return False
        ...
        if not result_success(exclusive_resp):
            log_error("TCID26_AudioMixingWorkflow Failed ❌ (exclusive-audio start rejected)")
            return False
    ...
    msg = "TCID26_AudioMixingWorkflow Passed ✅"
