"""
/**
 * @file TCID08_ReadWriteEdid.py
 * @brief L3 AVInput functional testcase.
 *
 * @testcase TCID08_ReadWriteEdid
...
def run_test():
    ...
    if not edid:
        log_error("TCID08_ReadWriteEdid Failed ❌ (readEDID returned empty/invalid EDID)")
        return False
    ...
    if not result_success(write_resp):
        log_error("TCID08_ReadWriteEdid Failed ❌ (writeEDID not accepted)")
        return False
    ...
    if not parse_edid(reread_resp):
        log_error("TCID08_ReadWriteEdid Failed ❌ (re-read EDID empty/invalid)")
        return False
    log_success("✅ EDID re-read after write")

    elapsed_time = time.perf_counter() - start_time
    msg = "TCID08_ReadWriteEdid Passed ✅"
