"""
/**
 * @file TCID25_SourceInspectionWorkflow.py
 * @brief L3 AVInput scenario testcase.
 *
 * @testcase TCID25_SourceInspectionWorkflow
...
def run_test():
    ...
    try:
        ...
        if not result_success(start_resp):
            log_error("TCID25_SourceInspectionWorkflow Failed ❌ (startInput rejected)")
            return False
        time.sleep(2)
        ...
        if not version:
            log_error("TCID25_SourceInspectionWorkflow Failed ❌ (HdmiCapabilityVersion missing)")
            return False
        ...
        if spd is None:
            log_error("TCID25_SourceInspectionWorkflow Failed ❌ (getSPD missing HDMISPD)")
            return False
        report["spd"] = spd
        ...
        if raw_spd is None:
            log_error("TCID25_SourceInspectionWorkflow Failed ❌ (getRawSPD missing HDMISPD)")
            return False
        ...
        if mode is None:
            log_error("TCID25_SourceInspectionWorkflow Failed ❌ (currentVideoMode not a string)")
            return False
        ...
        if protected is None:
            log_error("TCID25_SourceInspectionWorkflow Failed ❌ (isContentProtected not boolean)")
            return False
    ...
    msg = "TCID25_SourceInspectionWorkflow Passed ✅"
