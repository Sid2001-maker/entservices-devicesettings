"""
/**
 * @file TCID24_PresentationWindowWorkflow.py
 * @brief L3 AVInput scenario testcase.
 *
 * @testcase TCID24_PresentationWindowWorkflow
...
def run_test():
    ...
    try:
        ...
        if not result_success(start_resp):
            log_error("TCID24_PresentationWindowWorkflow Failed ❌ (startInput rejected)")
            return False
        ...
        for label, x, y, w, h in GEOMETRY_SEQUENCE:
            ...
            if not result_success(resp):
                log_error(f"TCID24_PresentationWindowWorkflow Failed ❌ ({label} rejected)")
                return False
        ...
        if not result_success(stop_resp):
            log_error("TCID24_PresentationWindowWorkflow Failed ❌ (stopInput rejected)")
            return False
        ...
        if not responded(post_resp):
            log_error("TCID24_PresentationWindowWorkflow Failed ❌ (no response after stop)")
            return False
    ...
    msg = "TCID24_PresentationWindowWorkflow Passed ✅"
