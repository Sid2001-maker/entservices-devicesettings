"""
/**
 * @file TCID21_GameModeProvisioning.py
 * @brief L3 AVInput scenario testcase.
 *
 * @testcase TCID21_GameModeProvisioning
...
def run_test():
    ...
    try:
        ...
        if not result_success(ver_resp):
            log_error("TCID21_GameModeProvisioning Failed ❌ (could not select EDID 2.0)")
            return False

        log_info("Step 2: enable Game Mode (ALLM advertised in EDID)")
        if not _toggle_allm(True):
            log_error("TCID21_GameModeProvisioning Failed ❌ (ALLM not enabled)")
            return False
        ...
        if parse_game_feature_mode(status_resp) is None:
            log_error("TCID21_GameModeProvisioning Failed ❌ (ALLM status not queryable)")
            return False
        log_success("✅ ALLM status queryable while Game Mode enabled")

        log_info("Step 4: disable Game Mode")
        if not _toggle_allm(False):
            log_error("TCID21_GameModeProvisioning Failed ❌ (ALLM not disabled)")
            return False
        ...
        if parse_game_feature_mode(status_off) is None:
            log_error("TCID21_GameModeProvisioning Failed ❌ (ALLM status unavailable when disabled)")
            return False
    ...
    msg = "TCID21_GameModeProvisioning Passed ✅"
