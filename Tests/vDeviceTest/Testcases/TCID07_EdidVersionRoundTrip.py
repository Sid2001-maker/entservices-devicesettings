"""
/**
 * @file TCID07_EdidVersionRoundTrip.py
 * @brief L3 AVInput functional testcase.
 *
 * @testcase TCID07_EdidVersionRoundTrip
...
def run_test():
    ...
    try:
        ...
        if not _set_and_verify(AVInputApis.EDID_VERSION_20):
            log_error("TCID07_EdidVersionRoundTrip Failed ❌ (HDMI2.0 round-trip mismatch)")
            return False
        ...
        if not _set_and_verify(AVInputApis.EDID_VERSION_14):
            log_error("TCID07_EdidVersionRoundTrip Failed ❌ (HDMI1.4 round-trip mismatch)")
            return False
    ...
    msg = "TCID07_EdidVersionRoundTrip Passed ✅"
