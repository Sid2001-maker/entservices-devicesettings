"""
/**
 * @file TCID17_InvalidParameterHandling.py
 * @brief L3 AVInput robustness testcase for malformed parameters.
 *
 * @testcase TCID17_InvalidParameterHandling
...
def run_test():
    ...
    for label, cmd in malformed_calls:
        ...
        if not responded(resp):
            log_error(f"TCID17_InvalidParameterHandling Failed ❌ (no response for {label})")
            return False
    ...
    if not is_ok(health):
        log_error("TCID17_InvalidParameterHandling Failed ❌ (plugin unresponsive after malformed input)")
        return False
    log_success("✅ Plugin responsive after malformed input")

    elapsed_time = time.perf_counter() - start_time
    msg = "TCID17_InvalidParameterHandling Passed ✅"
