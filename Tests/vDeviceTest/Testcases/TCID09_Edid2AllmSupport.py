"""
/**
 * @file TCID09_Edid2AllmSupport.py
 * @brief L3 AVInput functional testcase.
 *
 * @testcase TCID09_Edid2AllmSupport
...
def run_test():
    ...
    try:
        ...
        if not result_success(ver_resp):
            log_error("TCID09_Edid2AllmSupport Failed ❌ (could not select EDID 2.0)")
            return False

        log_info("Enabling ALLM in EDID on port 0")
        if not _set_allm_and_verify(True):
            log_error("TCID09_Edid2AllmSupport Failed ❌ (ALLM not enabled under EDID 2.0)")
            return False
        log_success("✅ ALLM-in-EDID enabled")

        log_info("Disabling ALLM in EDID on port 0")
        if not _set_allm_and_verify(False):
            log_error("TCID09_Edid2AllmSupport Failed ❌ (ALLM not disabled under EDID 2.0)")
            return False
    ...
    msg = "TCID09_Edid2AllmSupport Passed ✅"
