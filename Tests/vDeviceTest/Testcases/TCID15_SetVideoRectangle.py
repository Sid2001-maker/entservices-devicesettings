"""
/**
 * @file TCID15_SetVideoRectangle.py
 * @brief L3 AVInput functional testcase.
 *
 * @testcase TCID15_SetVideoRectangle
...
def run_test():
    ...
    try:
        ...
        if not is_ok(start_resp):
            log_error("TCID15_SetVideoRectangle Failed ❌ (startInput not accepted)")
            return False
        ...
        if not result_success(full_resp):
            log_error("TCID15_SetVideoRectangle Failed ❌ (full-screen rectangle not accepted)")
            return False
        ...
        if not result_success(pip_resp):
            log_error("TCID15_SetVideoRectangle Failed ❌ (PIP rectangle not accepted)")
            return False
    ...
    msg = "TCID15_SetVideoRectangle Passed ✅"
