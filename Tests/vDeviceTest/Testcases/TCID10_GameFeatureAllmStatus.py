"""
/**
 * @file TCID10_GameFeatureAllmStatus.py
 * @brief L3 AVInput functional testcase.
 *
 * @testcase TCID10_GameFeatureAllmStatus
...
def run_test():
    ...
    if features is None:
        log_error("TCID10_GameFeatureAllmStatus Failed ❌ (supported features list missing)")
        return False
    if AVInputApis.GAME_FEATURE_ALLM not in features:
        log_error("TCID10_GameFeatureAllmStatus Failed ❌ (ALLM not advertised as supported)")
        return False
    ...
    if mode is None:
        log_error("TCID10_GameFeatureAllmStatus Failed ❌ (ALLM status not a boolean)")
        return False
    log_success(f"✅ ALLM game feature status = {mode}")

    elapsed_time = time.perf_counter() - start_time
    msg = "TCID10_GameFeatureAllmStatus Passed ✅"
