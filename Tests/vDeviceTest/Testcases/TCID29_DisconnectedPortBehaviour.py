"""
/**
 * @file TCID29_DisconnectedPortBehaviour.py
 * @brief L3 AVInput scenario testcase.
 *
 * @testcase TCID29_DisconnectedPortBehaviour
...
def run_test():
    ...
    if devices is None:
        log_error("TCID29_DisconnectedPortBehaviour Failed ❌ (device list invalid)")
        return False

    connected = get_device_connected(devices, PORT)
    if connected is None:
        log_error("TCID29_DisconnectedPortBehaviour Failed ❌ (port 0 not present in device list)")
        return False
    ...
    try:
        ...
        if not result_success(start_resp):
            log_error("TCID29_DisconnectedPortBehaviour Failed ❌ (startInput rejected)")
            return False
        ...
        if mode is None:
            log_error("TCID29_DisconnectedPortBehaviour Failed ❌ (currentVideoMode not a string)")
            return False
        ...
        if not parse_edid(edid_resp):
            log_error("TCID29_DisconnectedPortBehaviour Failed ❌ (readEDID unavailable)")
            return False
        ...
        if not parse_hdmi_version(ver_resp):
            log_error("TCID29_DisconnectedPortBehaviour Failed ❌ (getHdmiVersion unavailable)")
            return False
    ...
    msg = "TCID29_DisconnectedPortBehaviour Passed ✅"
