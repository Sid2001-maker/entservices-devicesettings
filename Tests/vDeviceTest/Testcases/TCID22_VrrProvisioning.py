"""
/**
 * @file TCID22_VrrProvisioning.py
 * @brief L3 AVInput scenario testcase.
 *
 * @testcase TCID22_VrrProvisioning
...
def run_test():
    ...
    try:
        ...
        if not result_success(ver_resp):
            log_error("TCID22_VrrProvisioning Failed ❌ (could not select EDID 2.0)")
            return False

        log_info("Step 2: advertise VRR support")
        if not _toggle_vrr(True):
            log_error("TCID22_VrrProvisioning Failed ❌ (VRR support not enabled)")
            return False
        log_success("✅ VRR advertised in EDID")

        log_info("Step 3: withdraw VRR support")
        if not _toggle_vrr(False):
            log_error("TCID22_VrrProvisioning Failed ❌ (VRR support not disabled)")
            return False
    ...
    msg = "TCID22_VrrProvisioning Passed ✅"
