"""
/**
 * @file TCID27_NegativeAndBoundaryHandling.py
 * @brief L3 AVInput scenario testcase.
 *
 * @testcase TCID27_NegativeAndBoundaryHandling
...
def run_test():
    ...
    try:
        for label, cmd in malformed_calls:
            ...
            if not responded(resp):
                log_error(f"TCID27_NegativeAndBoundaryHandling Failed ❌ (no response for {label})")
                return False
        ...
        if not is_ok(health) or parse_number_of_inputs(health) is None:
            log_error("TCID27_NegativeAndBoundaryHandling Failed ❌ (plugin unhealthy after fuzzing)")
            return False
        ...
        if not parse_edid_version(good):
            log_error("TCID27_NegativeAndBoundaryHandling Failed ❌ (valid getter broken after fuzzing)")
            return False
    ...
    msg = "TCID27_NegativeAndBoundaryHandling Passed ✅"
